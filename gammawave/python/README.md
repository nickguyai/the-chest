...
6. **Configure Gemini API Key (Optional)**
   ...
   If you want to use Gemini audio transcription, set the `GOOGLE_API_KEY` environment variable:
...
### Real-time Transcription (OpenAI)
- **WebSocket:** `/api/v1/ws`
- Real-time audio streaming with OpenAI's GPT-4o model

### Audio File Transcription (Gemini)
- **POST:** `/api/v1/transcribe/gemini`
- Upload audio files for transcription with speaker identification
- Supports: WAV, MP3, OGG, FLAC
- Returns structured JSON with title, segments, and summary

Example usage:
```bash
curl -X POST "http://localhost:3005/api/v1/transcribe/gemini" \
...
- **POST:** `/api/v1/ask_ai` - Ask AI questions
